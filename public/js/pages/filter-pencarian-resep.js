...
const elements = {
    ...
    resepList:
        document.getElementById('resepList'),
...
function renderChips(list) {
    ...
    list.forEach(nama => {
        ...
        closeBtn.className =
            'material-icons-round chip-close';
...
function updateResultView(list) {
    ...
    const totalResep =
        elements.resepList
            .querySelectorAll('.resep')
            .length;

    if (totalResep === 0) {

        showEmpty(
            'Resep tidak ditemukan dengan bahan tersebut'
        );
    ...
    elements.resultInfo.textContent =
        `Menampilkan ${totalResep} resep dari: ${list.join(', ')}`;
